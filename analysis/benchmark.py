...
def refactor(df):
    if group == 'baselines':
        df = df.rename({'mappo': 'MAPPO', 'hdqn': 'HDQN', 'fixed': 'Always-on', 'simple1': 'Auto-SM1', 'simple2': 'Auto-SM2', 'simple': 'Simple'})
...
for scenario in vars_df.index.levels[1]:
    ...
    for key, ser in _sdf.items():
        print(scenario, key)
        if key in ["drop ratio", "power consumption (kW)", "energy efficiency (kb/J)"]:
            print(key)
...
columns_order = ['policy', 'scenario', 'power consumption (kW)', 'drop ratio', 'energy efficiency (kb/J)']
